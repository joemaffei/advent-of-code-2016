...
var parseInstruction = function(origin, instruction) {
  ...
  for (var i = 0; i < steps; i++) {
    stepLog.push({
      x: origin.x + movementMultiplier[newDirection].x * i,
      y: origin.y + movementMultiplier[newDirection].y * i,
    });    
  }
  return {
...
var firstDuplicate = function(steps) {
  return steps.filter(function(current, index){    
    var previousSteps = steps.slice(0, index);
    return previousSteps.filter(function(previous){
      return (previous.x === current.x) && (previous.y === current.y)
    })[0];
  })[0];
}

console.log(distanceFromOrigin(firstDuplicate(stepLog))) // 163
