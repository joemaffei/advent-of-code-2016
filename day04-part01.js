...
zsxyfgqj-hqfxxnknji-idj-xytwflj-359[jxfin]`.split(/\n/g);

var getName = function(room) {
    return room.match(/([a-z]+\-)+/g)[0].replace(/\-/g,'');
}

var getSectorID = function(room) {
    return +(room.match(/\d+/g)[0]);
}

var getChecksumArray = function(room) {
    return room.match(/\[[a-z]+/g)[0].replace(/\[/g, '').split('');
}

var nameAsArray = function(room) {
    return getName(room).split('');
}

var letterCounts = function(room) {
  return nameAsArray(room).reduce(function(acc, letter) {
    acc[letter] = (acc[letter] || 0) + 1;
    return acc;
  }, {});
}

var lettersByCount = function(room) {
  var counts = letterCounts(room);
  return Object.keys(letterCounts(room)).reduce(function(acc, letter) {
    if (acc[counts[letter]]) {
      acc[counts[letter]].push(letter);
      acc[counts[letter]].sort();
    } else {
      acc[counts[letter]] = [letter];
    }
    return acc;
  }, {});
}

var sortedCount = function(room) {
  var lbc = lettersByCount(room);
  var reverseKeys = Object.keys(lbc).sort(function(a, b){
    return a < b;
  });
  return reverseKeys.reduce(function(acc, key){
    acc = acc.concat(lbc[key]);
    return acc;
  }, []);
}
...
var realRooms = function(rooms) {
  return rooms.filter(function(room) {
    return validateName(room);
  });
}

var sumOfRealSectorIDs = function(rooms) {
  return realRooms(rooms).reduce(function(acc, room) {
    return acc + getSectorID(room);
  }, 0)
}
