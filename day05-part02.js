...
while (count < 8) {
  var hash = md5(input + seed);
  if (hash.substr(0,5) === '00000') {
    var position = hash.substr(5,1);
    if ('01234567'.indexOf(position) > -1 && password[position] === undefined) {
      var char = hash.substr(6,1);
      password[position] = char;
      count++;
      console.log('seed: ' + seed + ', hash: ' + hash + ', position: ' + position + ', char: ' + char);
    }
  }
  seed++;
}

console.log(password.join('')); // 863dde27
