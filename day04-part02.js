...
var roomAsArray = function(room) {
    return room.split('');
}


var shiftCharacter = function(character, shift) {
  var code = character.charCodeAt(0) - 97;
  if (character === '-') {
    return ' '
  } else if (code >= 0 && code <= 25) {
    var shiftedCode = (code + shift) % 26;
    return String.fromCharCode(97 + shiftedCode);
  } else {
    return character;
  }
}

var decypherRoom = function(room) {
  var shift = getSectorID(room);
  return roomAsArray(room).reduce(function(acc, character){
    return acc.concat(shiftCharacter(character, shift));
  }, '');
}

var decypherAllRooms = function(rooms) {
  return rooms.map(decypherRoom);
}

var findNorthPole = function(rooms) {
  return decypherAllRooms(rooms).filter(function(room) {
    return room.indexOf('north') < room.indexOf('pole');
  })[0];
}

var northPoleID = getSectorID(findNorthPole(rooms));

console.log(northPoleID); // 548
